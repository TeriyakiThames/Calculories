...
export default function DietaryForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const validationResult = userSchema.shape.dietary.safeParse(dietary);

    if (!validationResult.success) {
      setErrors({ dietary: validationResult.error.issues[0].message });
      return;
    }
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Failed to save dietary restrictions. Please try again.";
